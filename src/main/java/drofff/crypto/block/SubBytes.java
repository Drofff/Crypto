...
public class SubBytes {
	...
	private static final int[][] SUBSTITUTION_TABLE = new int[][] {
			{
				0x63, 0x7c, 0x77, 0x7b, 0xf2, 0x6d, 0x6f, 0xc5, 0x30, 0x1, 0x67, 0x2b, 0xfe, 0xd7, 0xab, 0x76
			},
			{
	...
	public static int[] substitute(int [] array) {
		int [] substitutedArray = new int[array.length];
		for(int i = 0; i < array.length; i++) {
			substitutedArray[i] = substituteByte(array[i]);
		}
		return substitutedArray;
	}

	private static int substituteByte(int elem) {
		int rowIndex = getRowIndexFromByte(elem);
		int columnIndex = getColumnIndexFromByte(elem);
		return SUBSTITUTION_TABLE[rowIndex][columnIndex];
	}
	...
	public static int[] inverseSubstitute(int [] array) {
		Integer[] arrayBoxed = ArrayUtils.inboxArray(array);
		int [] result = new int[array.length];
		for(int i = 0; i < SUBSTITUTION_TABLE.length; i++) {
			for(int j = 0; j < SUBSTITUTION_TABLE[0].length; j++) {
				int currentElement = SUBSTITUTION_TABLE[i][j];
				if(ArrayUtils.isElementInArray(currentElement, arrayBoxed)) {
					int position = ArrayUtils.indexOfElementInArray(currentElement, arrayBoxed);
					result[position] = getByteFromPosition(i, j);
				}
			}
		}
		return result;
	...
	private static int getByteFromPosition(int rowIndex, int columnIndex) {
		int shiftedRowIndex = rowIndex << 4;
		return shiftedRowIndex | columnIndex;
	}
